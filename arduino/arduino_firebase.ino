...
void calibrateIMU() {
  ...
  mpu.magUpdate();
  Serial.print("Magnetometer test - X: "); Serial.print(mpu.magX());
  Serial.print(" Y: "); Serial.print(mpu.magY());
  Serial.print(" Z: "); Serial.println(mpu.magZ());
...
void runCalibrationMode() {
  ...
  while (true) {
    ...
    Serial.print(" | Soil %: "); Serial.print(analogToPercentage(soilRaw, SOIL_DRY_VALUE, SOIL_WET_VALUE));
    Serial.print(" | Rain %: "); Serial.println(analogToPercentage(rainRaw, RAIN_DRY_VALUE, RAIN_WET_VALUE));
...
void readAndUpload() {
  ...
  Serial.print("Light Level: "); Serial.print(light, 1); Serial.println(" lx");
  ...
  Serial.print("  Z: "); Serial.println(accel[2], 3);
  
  Serial.println("--- Gyroscope (deg/s) ---");
  Serial.print("  X: "); Serial.print(gyro[0], 2);
  Serial.print("  Y: "); Serial.print(gyro[1], 2);
  Serial.print("  Z: "); Serial.println(gyro[2], 2);
  
  Serial.println("--- Magnetometer (uT) ---");
  Serial.print("  X: "); Serial.print(mag[0], 1);
  Serial.print("  Y: "); Serial.print(mag[1], 1);
  Serial.print("  Z: "); Serial.println(mag[2], 1);
  ...
  if(Firebase.ready()) {
    ...
    if (!Firebase.setInt(fbdo, "/soilMoisture", soilMoisture)) {
      success = false;
      Serial.print("Soil upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100); // Small delay between uploads
    
    if (!Firebase.setInt(fbdo, "/rain", rainLevel)) {
      success = false;
      Serial.print("Rain upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    
    if (!Firebase.setFloat(fbdo, "/light", light)) {
      success = false;
      Serial.print("Light upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    ...
    if (!Firebase.setFloat(fbdo, "/tilt", tilt)) {
      success = false;
      Serial.print("Tilt upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    
    if (!Firebase.setFloat(fbdo, "/accelX", accel[0])) {
      success = false;
      Serial.print("AccelX upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(150); // Increased delay for IMU stability
    
    if (!Firebase.setFloat(fbdo, "/accelY", accel[1])) {
      success = false;
      Serial.print("AccelY upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(150); // Increased delay for IMU stability
    
    if (!Firebase.setFloat(fbdo, "/accelZ", accel[2])) {
      success = false;
      Serial.print("AccelZ upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(150); // Increased delay for IMU stability
    ...
    if (!Firebase.setFloat(fbdo, "/gyroX", gyro[0])) {
      success = false;
      Serial.print("GyroX upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    
    if (!Firebase.setFloat(fbdo, "/gyroY", gyro[1])) {
      success = false;
      Serial.print("GyroY upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    
    if (!Firebase.setFloat(fbdo, "/gyroZ", gyro[2])) {
      success = false;
      Serial.print("GyroZ upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    ...
    if (!Firebase.setFloat(fbdo, "/magX", mag[0])) {
      success = false;
      Serial.print("MagX upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    
    if (!Firebase.setFloat(fbdo, "/magY", mag[1])) {
      success = false;
      Serial.print("MagY upload failed: "); Serial.println(fbdo.errorReason());
    }
    delay(100);
    
    if (!Firebase.setFloat(fbdo, "/magZ", mag[2])) {
      success = false;
      Serial.print("MagZ upload failed: "); Serial.println(fbdo.errorReason());
    }
